...
def calculate_entropy(file_path):
    """Menghitung entropy file untuk deteksi obfuscation."""
    try:
        ...
        logger.error(f"Error menghitung entropy: {e}")
        return None
...
def extract_file_metadata(file_path):
    ...
    try:
        ...
        metadata['ctime'] = os.path.getctime(file_path)
        metadata['mtime'] = os.path.getmtime(file_path)
        metadata['atime'] = os.path.getatime(file_path)
    except Exception as e:
        logger.warning(f"File time attr error: {e}")

    # Hashes
    metadata['md5'] = EradicationManager.calculate_file_hash_static(file_path, "md5")
    metadata['sha256'] = EradicationManager.calculate_file_hash_static(file_path, "sha256")
    # Entropy
    metadata['entropy'] = calculate_entropy(file_path)
    # Extension
    metadata['extension'] = os.path.splitext(file_path)[1].lower().replace('.', '')
    # MIME type
    if magic:
        try:
    ...
    # Machine / Entry point / IoC lain
    # --- PE ---
    if pefile and metadata['extension'] in ['exe', 'dll', 'sys']:
        try:
            ...
            metadata['has_signature'] = hasattr(pe, 'DIRECTORY_ENTRY_SECURITY')
        except Exception as e:
            logger.warning(f"PE parse error: {e}")
    # --- ELF / Mach-O ---
    elif lief and metadata['extension'] in ['elf', 'so', 'bin', 'out']:
        try:
            binary = lief.parse(file_path)
            metadata['machine'] = str(binary.header.machine_type) if hasattr(binary, 'header') else None
            metadata['entry_point'] = hex(binary.entrypoint) if hasattr(binary, 'entrypoint') else None
            metadata['imported_libraries'] = binary.libraries if hasattr(binary, 'libraries') else None
        except Exception as e:
            logger.warning(f"ELF/MachO parse error: {e}")
...
class EradicationManager:
    ...
    def calculate_file_hash(self, file_path, hash_alg="sha256"):
        return EradicationManager.calculate_file_hash_static(file_path, hash_alg)
    ...
    def quarantine_file(self, file_path, detection_reason="Unknown"):
        try:
            ...
            # --- IOC PATCH START
            metadata = extract_file_metadata(quarantine_path)
            metadata["detection_reason"] = detection_reason
            metadata["alert_context"] = self.current_alert_context
            ...
            if command == "scan-file" and len(sys.argv) > 2:
                ...
                if os.path.exists(file_path):
                    ...
                    if detections:
                        ...
                        # --- IOC PATCH: export metadata for manual scan
                        metadata = extract_file_metadata(file_path)
                        metadata_path = file_path + ".scan_metadata.json"
                        with open(metadata_path, 'w') as f:
                            json.dump(metadata, f, indent=2)
                        # --- IOC PATCH END
                        sys.exit(1)
